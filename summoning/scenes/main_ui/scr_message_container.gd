...
# stolen from greg rpg
# (my other game)
# (so its not actual stealing)
...
func push_message(text: String, options := {}) -> void:
	...
	lab["theme_override_font_sizes/font_size"] = 16
	lab["theme_override_colors/font_outline_color"] = Color.BLACK
	add_child(lab)
	last_y = get_children().map(func(a: Control): return a.position.y).max() + 16
